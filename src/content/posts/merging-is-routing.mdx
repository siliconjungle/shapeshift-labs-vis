...
```txt
apply: evidence is strong enough
review: evidence is useful but incomplete
rebase: the idea fits, but the head moved
rerun: the evidence is stale, failed, or too weak
split: one part is safe and another part is not
ask: a decision is missing
block: the candidate conflicts with the system
```
...
```txt
apply: private helper
review: public API
rerun: runtime effect proof
```
